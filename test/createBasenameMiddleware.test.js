...
describe('createBasenameMiddleware', () => {
  [
    ...
    describe(title, () => {
      ...
      it('should prepend basename to location descriptors', () => {
        expect(invokeMakeLocationDescriptor(basenameMiddleware, {
          pathname: '/path',
        })).to.eql({
          pathname: '/foo/path',
        });
      ...
      it('should strip basename from locations', () => {
        expect(invokeMakeLocation(basenameMiddleware, {
          pathname: '/foo/path',
        })).to.eql({
          pathname: '/path',
        });
  ...
  describe('trivial basename', () => {
    ...
    it('should not modify location descriptors', () => {
      ...
      expect(invokeMakeLocationDescriptor(basenameMiddleware, location))
        .to.equal(location);
    });

    it('should not modify locations', () => {
      const location = { pathname: '/path' };
      expect(invokeMakeLocation(basenameMiddleware, location))
        .to.equal(location);
    });
  });
});
